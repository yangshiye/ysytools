...
public final class ListUtil {
    ...
    public static <T> List<T> turn(Enumeration<T> enumeration) {
        if (enumeration == null) {
            return null;
        } else {
    ...
    public static <T> ArrayList<T> copyList(Collection collection) {
        if (collection == null) {
            return null;
        }
    ...
    public static void removeEmptyValue(Map map) {
        if (isNotEmpty(map)) {
            for (Object key : new HashSet<Object>(map.keySet())) {
                ...
                if (value == null) {
                    map.remove(key);
                } else if (value instanceof String && StrUtil.isEmpty(value.toString())) {
                    map.remove(key);
                }
            }
        }
    ...
    public static <K, V> void putNotEmpty(Map<K, V> map, K field, V obj) {
        if (obj == null) {
            return;
        } else if (obj instanceof String && StrUtil.isEmpty(obj.toString())) {
            return;
        }
        map.put(field, obj);
    }
